...
$sql = "SELECT h.id, h.action, DATE(h.actiondate), h.identifier, l.name, h.actionid FROM history AS h JOIN lists AS l ON l.id = h.actionid WHERE campaignid = {$id} and l.listid = 3 ORDER BY h.actiondate DESC LIMIT {$position}, {$item_per_page}";
...
while($row = mysqli_fetch_array($results)){
	...
	if ($counter == $row[5]) {
        ...
        echo '<li class="active" id="item_'.$row[0].'">
		            ...
		            <div class="timeline-content">
			            <p class="push-bit"><a href="page_ready_user_profile.php"><strong>'.$row[4].'</strong></a></p>
		            </div>
